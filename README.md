...
`@somnai-dreams/preimage/loading` exports `loadGallery`. Pass `aspects` when dimensions are already known; otherwise the helper probes dimensions through `PrepareQueue`. Separately, `imageLoading` controls when mounted tiles start visible image requests: `queued` caps visible image fetches and uses the virtual priority helpers to score mounted work by visible tiles, short-horizon predicted tiles, then scroll-direction distance; `visible-first` gates overscan work until the first viewport has loaded; `after-layout` waits until the frame layer is complete; and `immediate` starts image requests as tiles mount. The default is `queued`.
