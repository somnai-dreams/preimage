...
- **URL speed demo.** `/url-speed.html` accepts pasted HTTP(S) image URLs and reports time-to-dimensions, time-to-preview, source, and byte counts when available. It uses the same nav-level concurrency and probe-strategy controls as the other demos, cache-busting stays opt-in so signed URLs are not broken by default, and CORS-blocked fetch probes retry through the browser `<img>` path so arbitrary image hosts are still measurable.
