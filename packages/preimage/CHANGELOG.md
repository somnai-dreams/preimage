...
- **Virtual priority helpers.** `@somnai-dreams/preimage/virtual` now owns the viewport scoring primitives used by loading: `createVirtualPriorityTracker`, `createVirtualPriorityContext`, `getVirtualViewportRange`, `placementGapToRange`, `placementIntersectsRange`, `scoreVirtualPlacement`, and `virtualPlacementPriority`. They rank mounted work as visible, predicted, ahead, near, or behind, so gallery/resource schedulers can share the same virtualized priority model instead of reimplementing scroll math.
- **New subpath `@somnai-dreams/preimage/loading`.** `loadGallery({ imageLoading, aspects })` wraps the probe queue, virtual tile pool, and caller-owned render hooks without baking benchmark strategy names into the API. `aspects` means dimensions are already known and layout can commit without a probe phase. `imageLoading` controls visible image scheduling: `queued` places skeletons as dimensions arrive, caps visible image fetches, and uses the virtual priority helpers to score mounted work by visible tiles, short-horizon predicted tiles, then scroll-direction distance; `visible-first` gates overscan work until the first viewport has loaded; `after-layout` waits until the frame layer is complete; `immediate` starts image requests as tiles mount. The default is `queued`: render work starts at concurrency 2 while dimensions are still probing, rises to 4 for viewport work after layout completes, then can rise to 6 after the viewport has no pending image and scrolling has gone idle. That keeps the virtual-gallery default bounded and scroll-responsive without letting render-side image downloads starve dimension probes. `Gallery.done` waits for probes plus the initially visible image loads/errors, so benches and callers can record phase timings without racing the render-side fetches. Queued rendering tracks work by tile element, so a recycled DOM node never receives an old image request. The virtual demo is the reference consumer for image loading; the packing demo is now focused on shortest-column versus justified-row layout.
...
- **Remote loading strategy harness.** `bun run bench:remote-loading` drives the real loading orchestrator in Chromium against hosted demo photos with cache-busted URLs, scripted scroll, first-image/done timings, render-concurrency counts, estimated loaded image bytes, and visible pending-tile ratios. It warms the browser image origin with a tiny ranged fetch, runs one untimed one-image app pass before timed runs, records warmup metrics in saved JSON, and alternates strategy order across repeated runs so first-strategy connection/cache effects do not dominate comparisons. The default origin is GitHub Pages, and default `check:all` stays offline so routine local/CI runs do not spend hosted image bandwidth; set `PREIMAGE_CHECK_REMOTE_LOADING=1` to include the small remote `visible-first` versus `queued` pass deliberately.
- **Demo photo resource hints.** Demo pages now add `preconnect` and `dns-prefetch` hints when photo bytes come from a different origin, such as Vercel/custom-domain shells using the GitHub Pages photo host. The manual `Warm CDN` button remains opt-in and is still a warm-cache demo aid, not part of cold-load behavior.
- **URL speed demo.** `/url-speed.html` accepts pasted HTTP(S) image URLs and reports time-to-dimensions, time-to-preview, TTFB, download timing, selected strategy, source, and byte counts when available. It uses the same nav-level concurrency and probe-strategy controls as the other demos, cache-busting stays opt-in so signed URLs are not broken by default, and CORS-blocked fetch probes retry through the browser `<img>` path so arbitrary image hosts are still measurable.
- **Vercel demo photo bandwidth moved off preview deploys.** Vercel previews redirect `/assets/demos/photos/*` to the GitHub Pages deployment, demo URL generation uses that same asset root on `*.vercel.app` and `preimage.dearlarry.co`, and `build:demos:vercel` strips the heavy photo fixtures from the preview output after the normal demo build. Local dev and GitHub Pages keep same-origin asset URLs.
